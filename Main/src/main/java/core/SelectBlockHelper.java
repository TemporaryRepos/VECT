...
public class SelectBlockHelper {
    ...
    public static final int MAB_SELECT = 3;
    /**
    ...
    public static BlockInfo getIngredient(int method, Random random){
        ...
        if(method == MAB_SELECT){
            return getIngredientMAB(random);
        }
        return null;
    }
    /**
    ...
    private static BlockInfo getIngredientRWS(Random random){
        ...
        int classIndex = genRWSIndex(sumSucceedTable.toArray(new Double[0]),random,logInfoList);
        DTGlobal.getSelectLogger().info("第"+classIndex+"类被选择了");
//        int BlockIndex = genRWSIndex(succeedTable.get(classIndex).toArray(new Double[0]),random);
        int BlockIndex = random.nextInt(succeedTable.get(classIndex).size());
        return AllClusteringMutation.get(classIndex).get(BlockIndex);
    }
